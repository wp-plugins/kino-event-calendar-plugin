...
$query = "SELECT * FROM ".$table_prefix."ke_settings WHERE setting_id = 1";
$result = mysql_query($query);
$setting_event_colour = mysql_result($result, 0, "setting_event_colour");
$setting_event_colour_hover = mysql_result($result, 0, "setting_event_colour_hover");

$setting_event_colour = get_option("ke_setting_event_colour");
$setting_event_colour_hover = get_option("ke_setting_event_colour_hover");
?>
/*!
 * FullCalendar v1.4.1
...
function Grid(element, options, methods) {
	...
	function renderEvents(events) {
		...
		$(".fc-event a").hover(function()
		{
			var index = j(".fc-event a").index(this);
			j(".fc-event-hori:eq("+index+")").css("border-color", "<?php print $setting_event_colour_hover; ?>");	
		},
		function()
		{
			var index = j(".fc-event a").index(this);
			j(".fc-event-hori:eq("+index+")").css("border-color", "<?php print $setting_event_colour; ?>");	
		});
			...
			for (j=0; j<levels.length; j++) {
				...
				for (k=0; k<segs.length; k++) {
					...
					eventElement = $("<div class='" + className + event.className.join(' ') + "' />")
						//.append(eventAnchor = $("<a href='#' class='event-link "+event.id+"' title='"+event.title+"'/>")
						.append(eventAnchor = $("<a class='event-link "+event.id+"' title='"+event.title+"'></a>")
		...
		if (options.allDaySlot) {
			...
			for (i=0; i<len; i++) {
				...
				for (j=0; j<level.length; j++) {
					...
					eventElement = $("<div class='" + className + event.className.join(' ') + "' />")
						//.append(anchorElement = $("<a href='#' class='event-link "+event.id+"' title='"+event.title+"'/>")
						.append(anchorElement = $("<a class='event-link "+event.id+"' title='"+event.title+"'></a>")
							.append($("<span class='fc-event-title' />")
								//.text(event.title)));
								.html("&nbsp;")));
					if (event.url) {
